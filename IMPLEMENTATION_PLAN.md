...
| Phase 2 Search & Filtering | ❌ Not Started | SearchBar, FilterBar, FilterChip, etc. |
| Phase 3 Bulk Actions & Details | ❌ Not Started | BulkActionBar, IssueDetailPanel, etc. |
...
| `ui/src/components/filters/` | ❌ Not Started | FilterBar, FilterChip, FilterPreset, RangeSlider, DateRangePicker |
| `ui/src/components/search/` | ❌ Not Started | SearchBar, SearchHighlight |
| `ui/src/components/views/` | ❌ Not Started | GroupedView, SavedViews, ViewToggle |
| `ui/src/components/actions/` | ❌ Not Started | BulkActionBar, ActionConfirmDialog, ExportDialog |
| `ui/src/components/details/` | ❌ Not Started | IssueDetailPanel, IssueMetadata, CodeSnippet |
| `ui/src/components/dashboard/` | ❌ Not Started | Dashboard, StatCard, MiniChart, Charts |
| `ui/src/components/queue/` | ❌ Not Started | ProcessingQueue, QueueItem, QueueProgress |
| `ui/src/components/tags/` | ❌ Not Started | TagInput, TagBadge, TagManager |
| `ui/src/components/common/` | ❌ Not Started | KeyboardShortcuts, ShortcutsModal |
...
### Phase 2: Search & Filtering (P0)

#### 2.1 Search Functionality
**Priority:** P0 | **Effort:** Medium | **Directory:** `ui/src/components/search/`

No search exists. Implement per PRD-001:

- [ ] Create `SearchBar.tsx` - Input with clear button, scope selector (title/description/all)
- [ ] Create `SearchHighlight.tsx` - Highlight matching text in results
- [ ] Implement instant search (debounced 200ms)
- [ ] Add search history (last 10 in localStorage)
- [ ] Add keyboard shortcut `/` to focus search

**Current state:** Zero search functionality.
...
No filtering exists. Implement per PRD-001:

- [ ] Create `FilterBar.tsx` - Collapsible filter panel below header
- [ ] Create `FilterChip.tsx` - Active filter badges with remove button
- [ ] Create `FilterPreset.tsx` - Quick filter buttons (Critical Only, Security Only, High Volume, Recent)
- [ ] Create `RangeSlider.tsx` - Dual-handle range input for priority (0-100)
- [ ] Create `DateRangePicker.tsx` - Date range selection (First seen, Last seen)
- [ ] Implement filter state with URL query param sync
...
**Current state:** All issues shown in flat list, no filtering.

---

### Phase 3: Bulk Actions & Detail Panel (P0)

#### 3.1 Bulk Actions System
**Priority:** P0 | **Effort:** Medium | **Directory:** `ui/src/components/actions/`

Only basic "Process Selected" exists. Add per PRD-001:

- [ ] Create `BulkActionBar.tsx` - Sticky bar at bottom when items selected
- [ ] Create `ActionConfirmDialog.tsx` - Confirmation modal for destructive actions
- [ ] Create `ExportDialog.tsx` - Format and field selection for export
- [ ] Create `POST /api/issues/bulk` endpoint at `ui/src/app/api/issues/bulk/route.ts`

**Actions to implement:**
| Action | Confirmation Required |
|--------|----------------------|
| Select by Filter | No |
| Process Selected | No (existing) |
| Export Selected (CSV/JSON) | No |
| Set Priority | No |
| Add/Remove Tags | No |
| Ignore | Yes |
| Restore | No |

**Current state:** Only basic selection and process button.
...
No detail view exists. Implement per PRD-001:

- [ ] Create `IssueDetailPanel.tsx` - Slide-out panel (40% width, max 600px)
- [ ] Create `IssueMetadata.tsx` - Provider-specific metadata display
- [ ] Create `CodeSnippet.tsx` - Syntax-highlighted code preview
- [ ] Create `ActionButtons.tsx` - Quick action buttons
- [ ] Add keyboard navigation: Space to toggle, Escape to close, Arrow keys while open
...
**Current state:** Only table row - no expanded detail view.
...
No keyboard shortcuts exist. Implement per PRD-001:

- [ ] Create `ui/src/hooks/useKeyboardShortcuts.ts`
- [ ] Create `KeyboardShortcuts.tsx` - Event listener wrapper
- [ ] Create `ShortcutsModal.tsx` - Help dialog (`?` to open)
...
Only flat table view exists. Add per PRD-001:

- [ ] Create `GroupedView.tsx` - Container for grouped sections
- [ ] Create `GroupHeader.tsx` - Collapsible header with count badge
- [ ] Create `ViewToggle.tsx` - Switch between table/grouped views
- [ ] Implement group by: Provider, Severity, Date, Location, None
- [ ] Store collapsed groups in localStorage
- [ ] Add keyboard shortcuts: `c` collapse all, `e` expand all

---

#### 5.3 Dashboard with Statistics
**Priority:** P1 | **Effort:** Large | **Directory:** `ui/src/components/dashboard/`

No dashboard exists. Implement per PRD-001:

- [ ] Add `recharts` dependency to package.json
- [ ] Create `Dashboard.tsx` - Main collapsible dashboard section
- [ ] Create `StatCard.tsx` - Individual metric cards
- [ ] Create `MiniChart.tsx` - Compact inline charts
- [ ] Create `ProviderPieChart.tsx` - Issues by provider
- [ ] Create `SeverityBarChart.tsx` - Issues by severity
- [ ] Add toggle to show/hide dashboard
...
#### 5.4 Processing Queue Visualization
**Priority:** P1 | **Effort:** Medium | **Directory:** `ui/src/components/queue/`

Basic log viewer exists but no queue visualization. Add per PRD-001:

- [ ] Create `ProcessingQueue.tsx` - Queue visualization sidebar
- [ ] Create `QueueItem.tsx` - Individual item in queue
- [ ] Create `QueueProgress.tsx` - Overall progress bar with ETA
- [ ] Add cancel button for pending items
- [ ] Add pause/resume queue functionality
- [ ] Show current issue with spinner
...
No export exists. Implement:

- [ ] Create `export.ts` with CSV and JSON generation
- [ ] Implement client-side generation (no server round-trip)
- [ ] Add configurable column selection
- [ ] Filename format: `meta-ralph-issues-{date}.{format}`
